...
#
# Fish number P is represented by A[P] and B[P]. Array A contains the sizes of the fish. All its elements are
# unique. Array B contains the directions of the fish. It contains
# only 0s and/or 1s, where:
#
...
# meet each other. Then only one fish can stay alive ? the
# larger fish eats the smaller one. More precisely, we say that two fish P and Q meet each other when P < Q, B[P] = 1
...
# that, given two non-empty zero-indexed arrays A and B consisting of N integers, returns the number of fish that will
# stay alive.
#
